...
int main(){
    ...
    vector<MethodDef> lsDefs = {
        {"LS_STEEPEST_VSWAP_RAND",   NeighType::VERTEX_SWAP, LSMode::STEEPEST, true},
        {"LS_STEEPEST_ESWAP_RAND",   NeighType::EDGE_SWAP,   LSMode::STEEPEST, true},
        {"LS_GREEDY_VSWAP_RAND",     NeighType::VERTEX_SWAP, LSMode::GREEDY,   true},
        {"LS_GREEDY_ESWAP_RAND",     NeighType::EDGE_SWAP,   LSMode::GREEDY,   true},
        {"LS_STEEPEST_VSWAP_HEUR",   NeighType::VERTEX_SWAP, LSMode::STEEPEST, false},
        {"LS_STEEPEST_ESWAP_HEUR",   NeighType::EDGE_SWAP,   LSMode::STEEPEST, false},
        {"LS_GREEDY_VSWAP_HEUR",     NeighType::VERTEX_SWAP, LSMode::GREEDY,   false},
        {"LS_GREEDY_ESWAP_HEUR",     NeighType::EDGE_SWAP,   LSMode::GREEDY,   false},
    };
    ...
    for(size_t ii=0;ii<insts.size();ii++) {
        ...
        // RandomWalk z czasem = maxAvgTime (najwolniejsza metoda LS)
        cerr << "[" << tag << "] RandomWalk time limit = " << maxAvgTime*1000 << "ms\n";
        {
            RandomWalk rw(maxAvgTime);
            vector<int> objs; vector<double> times; vector<Solution> allSols;
            Solution best; bool hasBest=false;
            for(int rep=0;rep<100;rep++){
                auto t0=chrono::steady_clock::now();
                Solution s=rw.solve(inst);
                double dt=chrono::duration<double>(chrono::steady_clock::now()-t0).count();
                objs.push_back(s.objective()); times.push_back(dt);
                allSols.push_back(s);
                if(!hasBest||s.objective()>best.objective()){best=s;hasBest=true;}
            ...
            double avgT=accumulate(times.begin(),times.end(),0.0)/times.size();
            stats<<"RANDOM_WALK,"<<accumulate(objs.begin(),objs.end(),0.0)/objs.size()<<","
                 <<*min_element(objs.begin(),objs.end())<<","<<*max_element(objs.begin(),objs.end())<<","
                 <<avgT*1000<<","<<*min_element(times.begin(),times.end())*1000<<","<<*max_element(times.begin(),times.end())*1000<<"\n";
            saveBest(base+"/solutions/RANDOM_WALK.txt",best);
            saveAllCSV(base+"/solutions_all/RANDOM_WALK.csv",allSols,times);
        }
